...
// H-026B implementation PR can drop the engine, scheduler,
...
//   - engine.go — streaming pass under WithTxLockedOwnership(orgID) at REPEATABLE READ.
//   - explanation.go — winning rule + losing-rules snapshot.
//   - service.go — Recompute / Preview / ApplyOverride orchestrator.
//   - scheduler.go — sibling of findings.Scheduler; one goroutine per process, per-org sweep, ctx-cancel honored.
//
...
//     tags, ...). It reads them through a small consumer-owned
//     reader interface declared next to the engine in this
//     package, never in identity or governance itself.
...
//   - ownership -> inventory (reads via a consumer-owned interface).
//   - ownership -> identity (reads via a consumer-owned interface; identity is the producer of the vocabulary, governance reads it).
//   - ownership -> httpapi (reverse layering).
//   - ownership -> storage/postgres (must go through the governance.* interfaces in the parent package).
//
...
// PR sized this at < 1500 LOC; the engine is the largest single
// piece of H-026 work and lives here in isolation so review
// attention concentrates on the determinism and audit-atomicity
// invariants.
package ownership
